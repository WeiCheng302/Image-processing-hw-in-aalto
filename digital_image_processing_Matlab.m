...
figure(1),imshow(RGBimage),impixelinfo
impixel(RGBimage,100, 100)

%turn gray
figure(2),imshow(RGBimage),impixelinfo
figure(3),imshow(RGBimage(:,:,1))
grayimage=rgb2gray(RGBimage);
figure(4),imshow(grayimage)

%gray level
grayimage16=grayslice(grayimage, 16);
imshow(grayimage16)
grayimage4=grayslice(grayimage, 4);
imshow(grayimage4)
figure(5), imhist(grayimage16),axis([-50 300 0 max(imhist(grayimage16))])
figure(6), imhist(grayimage4),axis([-50 300 0 max(imhist(grayimage4))])
figure(7),imshow(histeq(grayimage16))
figure(8),imshow(histeq(grayimage4))


mycolormap=[0.5 0 1; 1 0.5 1;0 1 0.5; 1 0.5 0.5];
figure(9),imshow(grayimage4,mycolormap),impixelinfo
%binary image
...
%noise image
noiseimage=imnoise(grayimage,'salt & pepper');
imshow(noiseimage)

d_grayimage=im2double(grayimage);
imshow(d_grayimage)
newimage=1-d_grayimage;
imshow(newimage)
imshow(imsubtract(grayimage,128))
imshow(imadd(grayimage,128))
imshow(imdivide(grayimage,2))
imshow(immultiply(RGBimage,2))
...
imshow(immultiply(grayimage,binimage2))
...
figure(12),imshow(mat2gray(log(1+abs(f_image)))),impixelinfo

[x,y]=meshgrid(-256:255,-256:255);
z=sqrt(x.^2+y.^2);
c=z<15;
lowpass_f_image=f_image.*c;
figure(13),imshow(mat2gray(log(1+abs(lowpass_f_image)))),impixelinfo
...
gaussianfilter=mat2gray(fspecial('gaussian',512,20));
